...
#[tauri::command]
pub fn set_book_tags(state: State<AppState>, book_id: i64, tag_ids: Vec<i64>) -> Result<(), AppError> {
    state.repo.set_tags(book_id, tag_ids)
}

#[tauri::command]
pub fn increment_reread(state: State<AppState>, book_id: i64) -> Result<Book, AppError> {
    state.repo.increment_reread(book_id)
}
